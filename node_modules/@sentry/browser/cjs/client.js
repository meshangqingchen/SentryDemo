...
class BrowserClient extends core.BaseClient {
   ...
   */
   constructor(options) {
    ...
    if (options.sendClientReports && utils.WINDOW.document) {
      utils.WINDOW.document.addEventListener('visibilitychange', () => {
        if (utils.WINDOW.document.visibilityState === 'hidden') {
          this._flushOutcomes();
        }
      });
    }
   ...
   */
   _flushOutcomes() {
    ...
    try {
      const isRealNavigator = Object.prototype.toString.call(utils.WINDOW && utils.WINDOW.navigator) === '[object Navigator]';
      const hasSendBeacon = isRealNavigator && typeof utils.WINDOW.navigator.sendBeacon === 'function';
      // Make sure beacon is not used if user configures custom transport options
      if (hasSendBeacon && !this._options.transportOptions) {
        // Prevent illegal invocations - https://xgwang.me/posts/you-may-not-know-beacon/#it-may-throw-error%2C-be-sure-to-catch
        const sendBeacon = utils.WINDOW.navigator.sendBeacon.bind(utils.WINDOW.navigator);

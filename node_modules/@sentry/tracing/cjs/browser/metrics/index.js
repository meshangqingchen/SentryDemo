...
function getBrowserPerformanceAPI() {
  return utils.WINDOW && utils.WINDOW.addEventListener && utils.WINDOW.performance;
}
...
function startTrackingWebVitals(reportAllChanges = false) {
  ...
  if (performance && utils.browserPerformanceTimeOrigin) {
    if (performance.mark) {
      utils.WINDOW.performance.mark('sentry-tracing-init');
    }
    _trackCLS();
...
function addPerformanceEntries(transaction) {
  const performance = getBrowserPerformanceAPI();
  if (!performance || !utils.WINDOW.performance.getEntries || !utils.browserPerformanceTimeOrigin) {
  ...
  performanceEntries.slice(_performanceCursor).forEach((entry) => {
    ...
    switch (entry.entryType) {
      ...
      case 'resource': {
        const resourceName = (entry.name ).replace(utils.WINDOW.location.origin, '');
...
function _trackNavigator(transaction) {
  const navigator = utils.WINDOW.navigator ;
  if (!navigator) {
    return;
  }
